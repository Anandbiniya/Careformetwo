(function () {
	...
	$.extend(erpnext.utils, {
				...
				callback: function(r) {
					if (r.message && r.message.length) {
						r.message.forEach(function (dimension) {
							var existing_filter = filters.filter(function (el) { return el.fieldname === dimension['fieldname']; });
		...
		**/
		...
		check_payments_app: function () {
			if (frappe.boot.versions && !frappe.boot.versions.payments) {
				var marketplace_link = '<a href="https://frappecloud.com/marketplace/apps/payments">Marketplace</a>';
				var github_link = '<a href="https://github.com/frappe/payments/">GitHub</a>';
				var msg = __("payments app is not installed. Please install it from {0} or {1}", [marketplace_link, github_link]);
				frappe.msgprint(msg);
			}
					...
					if (frm.doc.status !== 'Closed' && frm.doc.service_level_agreement
						...
						frappe.call({
							...
							callback: function(data) {
								var statuses = data.message.pause_sla_on;
								var hold_statuses = [];
								$.each(statuses, function (_i, entry) {
									hold_statuses.push(entry.status);
								});
								if (hold_statuses.includes(frm.doc.status)) {
									frm.dashboard.clear_headline();
									var message = {'indicator': 'orange', 'msg': __('SLA is on hold since {0}', [moment(frm.doc.on_hold_since).fromNow(true)])};
						...
						var agreement_status = (frm.doc.agreement_status == 'Fulfilled') ?
	...
	function get_time_left(timestamp, agreement_status) {
		...
		var diff_display = diff >= 44500 ? moment.duration(diff).humanize() : 'Failed';
		var indicator = (diff_display == 'Failed' && agreement_status != 'Fulfilled') ? 'red' : 'green';
		return {'diff_display': diff_display, 'indicator': indicator};
	}

	function get_status(expected, actual) {
		var time_left = moment(expected).diff(moment(actual));
		if (time_left >= 0) {
	...
	*/
	...
	erpnext.taxes_and_totals = class TaxesAndTotals extends erpnext.payments {
		...
		_calculate_taxes_and_totals() {
			var is_quotation = this.frm.doc.doctype == "Quotation";
		...
		calculate_item_values() {
			...
			if (!this.discount_amount_applied) {
				for (var i = 0, list = this.frm._items || []; i < list.length; i += 1) {
					var item = list[i];
		...
		filtered_items() {
			return this.frm.doc.items.filter(function (item) { return !item["is_alternative"]; });
		}
	};
	...
	erpnext.TransactionController = class TransactionController extends erpnext.taxes_and_totals {
		setup() {
			...
			if(this.frm.fields_dict["items"].grid.get_field('batch_no')) {
				...
				var batch_field = this.frm.get_docfield('items', 'batch_no');
				if (batch_field) {
		...
		onload() {
			...
			if(this.frm.doc.__islocal) {
				...
				return frappe.run_serially([
					...
					function () { return set_value('is_subcontracted', 0); },
					function () {
		...
		scan_barcode() {
			var barcode_scanner = new erpnext.utils.BarcodeScanner({frm:this.frm});
			barcode_scanner.process_scan();
		}

		validate_has_items () {
			var table = this.frm.doc.items;
		...
		qty(doc, cdt, cdn) {
			...
			// item.pricing_rules = ''
			frappe.run_serially([
				function () { return this$1.remove_pricing_rule(item); },
				function () { return this$1.conversion_factor(doc, cdt, cdn, true); },
				function () { return this$1.calculate_stock_uom_rate(doc, cdt, cdn); },
				function () { return this$1.apply_pricing_rule(item, true); }
			]);
		}
		...
		_set_values_for_item_list(children) {
			...
			for (var i$1 = 0, list$1 = children; i$1 < list$1.length; i$1 += 1) {
				...
				var existing_pricing_rule = frappe.model.get_value(child.doctype, child.name, "pricing_rules");

				for (var i = 0, list = Object.entries(child); i < list.length; i += 1) {
					var ref = list[i];
					var key = ref[0];
					var value = ref[1];
		...
		apply_product_discount(args) {
			...
			args.free_item_data.forEach(function (pr_row) {
				...
				this$1.frm.script_manager.copy_from_first_row("items", row_to_modify, ["expense_account", "income_account"]);
			});
	...
	$.extend(erpnext.bulk_transaction_processing, {
		create: function(listview, from_doctype, to_doctype) {
			...
			var count_of_rows = checked_items.length;
			frappe.confirm(__("Create {0} {1} ?", [count_of_rows, to_doctype]), function (){
